...
UPDATE — 2026-09-25, source-verified: this is no longer open. `enum class SourceType(val persistedValue: String) { MANUAL, FTC, FCC }` now exists in DatabaseEntities.kt; PROTECTED_SOURCE_TYPES and isManualSource() both resolve through it, and a repo-wide search found no remaining priority==100 usage. The paragraph below describes the shape that was implemented; it is retained for historical context, not as an open item.
Required semantics:
Text
