...
/**
 * Extension to retrieve a source by name — needed by ensureSourceRow.
 * Delegates to getAllSources() Flow via a single collect emission.
 */
private suspend fun DataSourceRepository.getSourceByName(name: String): SourceEntity? {
    var result: SourceEntity? = null
    getAllSources().collect { sources ->
        result = sources.find { it.name == name }
    }
    return result
}
