...
UPDATE — 2026-09-25, verified directly against live source, not assumed: the SourceType enum now exists (DatabaseEntities.kt: `enum class SourceType(val persistedValue: String) { MANUAL, FTC, FCC }`). PROTECTED_SOURCE_TYPES and isManualSource() both now resolve through it; a repo-wide search of android/app/src/main/java found zero remaining `priority == 100` occurrences. This closes the hygiene item described above as "someday." Separately, the FCC Never/0/Unknown symptom this entry connects to was root-caused and fixed 2026-09-01 (SecureCsvParser.streamColumnByHeader() reading the "Caller ID Number" header instead of column zero) — confirmed present in live ReliableSourceManager.kt. The Contacts-wiring concern was traced 2026-09-19 and confirmed wired end-to-end (ContactsViewModel -> SecurityRuleRepository.addContactsAllowBatch() -> DataSourceRepository.insertEntriesAuthoritative()/rebuildDerivedIndexes()) — confirmed present in live source. Not yet re-confirmed: a real-device Sync Now tap on a MANUAL-type source, post-fix, to visually verify the original Never/0/Unknown symptom no longer appears — this remains the one open item from this entry. Also flagged during this pass: ReliableSourceManager.kt's FCC-endpoint doc comment had claimed neither FCC URL was live-verified; corrected to reflect the actual 2026-09-01 finding (primary URL returned no text and did not resolve; only the fallback URL was confirmed live) rather than leaving a stale blanket caution.
...
UPDATE — 2026-09-25, verified directly against live source: this is CLOSED. `DataSourceRepository.insertEntriesAuthoritative()` (DB only, never touches Bloom) and `rebuildDerivedIndexes()` (Bloom only) are separate suspend functions. `SecurityRuleRepository.replaceSourceSnapshot()` and `replaceSourceSnapshotBatched()` both call `insertEntriesAuthoritative()` strictly inside `database.withTransaction { ... }`, then call `rebuildDerivedIndexes()` afterward, outside the transaction, with the ordering explained in an inline comment ("This call is intentionally outside the transaction. The Room commit above establishes authoritative state before any Bloom mutation."). The required rollback test exists and passes the described scenario: `SourceActivationTransactionTest.batchedReplacement_rollsBackAllBatchesWhenProducerFails` (a producer failure after the first batch leaves the prior authoritative entry intact and the candidate entry absent), and `failedReplacement_preservesLastKnownGoodAndRecordsAttempt` separately confirms a failed candidate never becomes a decision-relevant Bloom false positive. This item's Phase 4.9.D test entry below already carried a "✅ COMPLETE" mark for the same evidence — the exit-criteria checkbox and Phase 9 gate line had simply not been updated to match; both are corrected below.
...
UPDATE — 2026-09-25, verified directly against live source: this is CLOSED. `ReliableSourceManager.syncAllFederalSources()` filters `SOURCES` through `shouldSyncAutomatically(dataSourceRepository.getSourceByName(source.name))` before syncing, where `shouldSyncAutomatically` returns `source?.isEnabled != false` (so a missing row is still eligible for first-run seeding, matching the inline comment's stated intent). The explicit single-source `syncSource(sourceId)` path deliberately does not apply this filter, so a manual refresh still works on a disabled source, as intended. `ReliableSourceManagerDisabledSourceTest` exists and covers this. This item's Phase 4.9.E test entry below described the same behavior without a completion mark — corrected below.
...
UPDATE — 2026-09-25, verified directly against live source: this is PARTIALLY resolved, not fully closed — do not mark it complete. Resolved: the SharedPreferences/apply()-then-navigate race is gone. `OnboardingWizardScreen` no longer touches SharedPreferences directly; `OnboardingViewModel.markEulaAccepted(version)` performs the writes via suspend calls to `SettingRepository`, and the screen's navigation is gated on the ViewModel's `eulaAccepted` StateFlow only flipping true after the writes succeed — a failure leaves it false and surfaces `eulaAcceptError` instead. `OnboardingViewModelEulaTest` covers both the success path and a total-persistence-failure path (`markEulaAccepted_persistenceFailureLeavesStateFalseAndSurfacesError`), satisfying the literal 4.9.G exit test ("write fails -> onboarding does not advance").
Partial-write behavior is now characterized by `OnboardingViewModelEulaTest.markEulaAccepted_versionWriteFailureLeavesAcceptedFlagPersistedWithoutMetadata`: the fake DAO throws specifically on the `EULA_VERSION` write after the earlier `EULA_ACCEPTED` write succeeds. The test asserts that the ViewModel remains unaccepted and surfaces an error while the persisted accepted flag is `true` and both version and timestamp are absent. The test passed in [Pulse Consumer CI run 36205908886](https://github.com/avignonbo-jpg/Signal-Gate-Pulse/actions/runs/36205908886) on commit `e81bf5a595d86b359d3cd285739a8a5b8212c76b` (`Run Unit Tests (Pulse)` succeeded). This documents the current inconsistency; it does not resolve it.

Still open, genuinely not done: the "coherent record, not three unrelated preference keys" requirement was not implemented. `markEulaAccepted()` still performs three independent `settingRepository.setSetting()` calls (`EULA_ACCEPTED`, `EULA_VERSION`, `EULA_ACCEPTED_AT`) rather than one atomic write of a single record, and there is no `agreement_id` field. A storage-shape decision (single serialized record, transactional multi-key write, or formally descope the requirement) remains outstanding. This is a product/design decision, not a mechanical fix — do not silently pick one and implement it.

Design recommendation — 2026-09-25: prefer option (a), storing `agreement_id`, `agreement_version`, and `accepted_at` as one serialized acceptance record under a single setting key. This most directly satisfies the stated coherent-record requirement and makes acceptance metadata one persistence unit. Specify the record format/version and compatibility or migration behavior for the existing three keys before implementation. This is a recommendation only, not product approval or authorization to change code/schema; 4.0.5 remains open pending an explicit choice. Option (b) would make the three writes transactional but retain separate records; option (c) would require an explicit product decision to descope and downgrade the requirement.
...
- [x] SourceType enum exists and priority==100 is no longer used as source identity anywhere — source-verified 2026-09-25
- [x] FCC source sync failure root-caused (bad URL vs. silent fetch failure) and either fixed or explicitly documented as not-yet-implemented — root cause was column mis-selection, not URL; fixed via streamColumnByHeader(), source-verified 2026-09-25. Note: primary FCC URL's own live resolution is still unconfirmed (see 4.0.2 update above) — this criterion covers the parsing defect, not endpoint uptime.
- [x] Contacts import confirmed wired to DataSourceRepository/sync-status, or confirmed broken and tracked as its own fix — confirmed wired, source-verified 2026-09-25. Real-device re-test of the Sync Now UI symptom post-fix is still outstanding.
- [x] Bloom mutation is provably post-commit-only (insertEntriesAuthoritative/rebuildDerivedIndexes split, with a rollback test) — source-verified 2026-09-25; see 4.9.D
...
Disabled-source sync test — **✅ COMPLETE**, source-verified 2026-09-25. A disabled source is skipped by the sync worker's automatic path (not a manual refresh); decision output is unaffected either way. `ReliableSourceManagerDisabledSourceTest` covers automatic skipping of disabled rows, manual-sync routing independent of enablement, and missing-row eligibility for first-run seeding. Covers 4.0.4. No fresh CI run URL is attached to this mark — source and test-file presence only.
...
EULA persistence-failure test — **✅ COMPLETE, CI-verified 2026-09-25.** `OnboardingViewModelEulaTest.markEulaAccepted_persistenceFailureLeavesStateFalseAndSurfacesError` proves a total persistence failure leaves acceptance false and surfaces an error. `markEulaAccepted_versionWriteFailureLeavesAcceptedFlagPersistedWithoutMetadata` additionally characterizes a partial failure: the accepted flag persists, the ViewModel remains unaccepted and surfaces an error, and version/timestamp are absent. Both run in [Pulse Consumer CI run 36205908886](https://github.com/avignonbo-jpg/Signal-Gate-Pulse/actions/runs/36205908886), whose `Run Unit Tests (Pulse)` step succeeded on commit `e81bf5a`. This completes the 4.9.G test evidence, but not 4.0.5 as a whole: the coherent-record requirement remains unresolved pending a product/design decision. See the 4.0.5 update above.
...
- [x] Bloom mutation is provably post-commit-only, with a rollback/contamination test passing (4.0.3 / 4.9.D) — source-verified 2026-09-25
- [x] SourceType is the enforced source-identity discriminator; priority is not used for identity anywhere (4.0.2) — source-verified 2026-09-25
