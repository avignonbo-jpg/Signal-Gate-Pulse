...
0.8 Add regression tests for all Phase 0 invariants — ☐ OPEN (partial sources now present, including mutation-boundary coverage)
Phase 0 only actually exits once tests exist and pass in mandatory CI — not when the code merely compiles and looks right on read-through. The live branch now contains Phase 0.2 Bloom-authority coverage, Phase 0.3 protected-source deletion coverage, Phase 0.4 migration/rollback coverage sources, Phase 0.5 CSV and bounded XLSX hard-failure coverage, Phase 0.6 explicit-failure coverage, focused SecurityRuleRepository mutation-boundary coverage, and four CallActionReceiver edge-action behavior tests. Mandatory execution evidence and generated schema version 3 still need to be confirmed before this gate can close.
...
[x] Mandatory CI workflow exists for the Phase 0 instrumented tests; execution of the complete expanded JVM/instrumented behavioral matrix remains evidence-gated
