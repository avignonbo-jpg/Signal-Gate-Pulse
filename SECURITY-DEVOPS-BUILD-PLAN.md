...
Still open, genuinely not done: the "coherent record, not three unrelated preference keys" requirement was not implemented. `markEulaAccepted()` still performs three independent `settingRepository.setSetting()` calls (`EULA_ACCEPTED`, `EULA_VERSION`, `EULA_ACCEPTED_AT`) rather than one atomic write of a single record, and there is no `agreement_id` field. The existing test only exercises total failure (the fake DAO throws on every call, including the first), not a partial-failure case where, say, the first `setSetting` call succeeds and a later one throws — which is exactly the scenario "coherent record" was meant to rule out. Until that's tested, an inconsistent partial-write state (e.g. `EULA_ACCEPTED=true` with no version or timestamp) is unverified, not disproven. This is a design decision about the storage shape (single JSON-like record vs. keeping three keys but writing them transactionally), not a mechanical fix — do not silently pick one and implement it.
...
EULA persistence-failure test — **✅ COMPLETE for the literal exit test**, source-verified 2026-09-25. `OnboardingViewModelEulaTest.markEulaAccepted_persistenceFailureLeavesStateFalseAndSurfacesError` proves persistence write fails -> onboarding does not advance. This closes the 4.9.G test itself but not 4.0.5 as a whole: the "coherent record, not three unrelated preference keys" requirement is still unimplemented, and only total-failure (not partial-failure across the three writes) is tested. See the 4.0.5 update above.
