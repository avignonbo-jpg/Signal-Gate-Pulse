...
Phase 0 only actually exits once tests exist and pass in mandatory CI — not when the code merely compiles and looks right on read-through. The live branch now contains Phase 0.2 Bloom-authority coverage, Phase 0.3 protected-source deletion coverage, Phase 0.4 migration/rollback coverage sources, Phase 0.5 CSV and bounded XLSX hard-failure coverage, and a Phase 0.6 explicit-failure regression source. Mandatory execution evidence, generated schema version 3, and remaining edge-action behavioral coverage still need to be confirmed before this gate can close; focused SecurityRuleRepository mutation-boundary sources are now present.
