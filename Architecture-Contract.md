...
One authoritative path for every decision-affecting mutation: see INV-001 and §5.2. Confirmed this session: DataSourceRepository.insertEntriesAuthoritative() is the authoritative DB-write chokepoint, while rebuildDerivedIndexes() is the explicit post-commit Bloom rebuild; SecurityRuleRepository is the single application-layer entry point above it for manual mutation, snapshot replacement, and sync-attempt/failure recording.
...
0.3 — [COMPLETE, source-confirmed this session, not independently CI-reconfirmed]. Protected source lifecycle semantics defined and enforced (PROTECTED_SOURCE_TYPES, ProtectedSourceDeletionException); DataSourceRepositoryDeletionTest (test) is the guard-level evidence. SourceDeletionCascadeTest (androidTest) covers foreign-key cascade behavior only, not the protected-source guard.
...
0.5 — [COMPLETE, source-confirmed this session, not independently CI-reconfirmed]. Resource-limit violations are hard failures in DataSyncEngine (parser throws rather than returns partial data); matching DataSyncEngineXlsxLimitTest (test) exists. Scope is the DataSyncEngine parsing layer specifically, covering its CSV and XLSX paths, not every parser that may be added elsewhere.
