...
Revision note (2026-08-13, reconciled): This draft merges two independent v3 lineages produced the same day and reconciles both against the actual current source, per the shared governance principle both lineages independently arrived at: if implementation reality and documentation disagree, do not silently choose one — record the discrepancy, inspect the actual code, and update accordingly.
...
This reconciled draft takes Lineage B's structure and invariant set (materially stronger) and corrects the specific items that Lineage B couldn't have known were already fixed. Everything else in Lineage B is adopted as-is. This is still a draft — neither lineage has been formally adopted over the currently-canonical v2 Architecture-Contract.md.
1. Scope
...
This is a draft revision (v3, reconciled) for review before being adopted as canonical over the currently-canonical v2 Architecture-Contract.md. Once approved, it should be committed as Architecture-Contract.md and the -v3-DRAFT suffix dropped. PROJECT_LEDGER.md should log the supersession. Historical amendment tracking from v2's footer (Amendments 0–5, applied 2026-07-15) is preserved in PROJECT_LEDGER.md and not repeated here.
